...
public final class MiscellaneousAlertPoster {
   ...
   public static void postMiscTp(BaseComponent hover) {
      Karhu.getInstance().getAlertsManager().getMiscDebugToggled().stream().map(Bukkit::getPlayer).filter(Objects::nonNull).forEach((admin) -> {
         admin.spigot().sendMessage(hover);
      });
   }
   ...
   public static void postMitigation(String msg) {
      Karhu.getInstance().getAlertsManager().getMitigationToggled().stream().map(Bukkit::getPlayer).filter(Objects::nonNull).forEach((admin) -> {
         admin.sendMessage(msg);
      });
   }

   public static void postSetback(String msg) {
      Karhu.getInstance().getAlertsManager().getSetbackToggled().stream().map(Bukkit::getPlayer).filter(Objects::nonNull).forEach((admin) -> {
         admin.sendMessage(msg);
      });
   }
   ...
   public static void handleDiscord(String data, KarhuPlayer pdata) {
      ...
      if (showIcon) {
         discord.addEmbed((new Webhook.EmbedObject()).setTitle("```" + player.getName() + "``` | Crash ").setThumbnail("https://minotar.net/avatar/" + player.getName() + "/50.png").setDescription(ChatColor.stripColor(data.replaceAll("\n", ""))).setColor(Color.ORANGE).addField("Info", (showWorld ? "World: " + player.getWorld().getName() + " | Coords: " + format(1, player.getLocation().getX()) + "/" + format(1, player.getLocation().getY()) + "/" + format(1, player.getLocation().getZ()) : " Coords: " + format(1, player.getLocation().getX()) + "/" + format(1, player.getLocation().getY()) + "/" + format(1, player.getLocation().getZ())) + (showStats ? " | TPS: " + Karhu.getInstance().getTPS() + " | Ping: " + pdata.getTransactionPing() + "ms" : ""), false).addField("Date", dtf.format(now), false));
      } else {
         discord.addEmbed((new Webhook.EmbedObject()).setTitle("```" + player.getName() + "``` | Crash ").setDescription(ChatColor.stripColor(data.replaceAll("\n", ""))).setColor(Color.ORANGE).addField("Info", (showWorld ? "World: " + player.getWorld().getName() + " | Coords: " + format(1, player.getLocation().getX()) + "/" + format(1, player.getLocation().getY()) + "/" + format(1, player.getLocation().getZ()) : " Coords: " + format(1, player.getLocation().getX()) + "/" + format(1, player.getLocation().getY()) + "/" + format(1, player.getLocation().getZ())) + (showStats ? " | TPS: " + Karhu.getInstance().getTPS() + " | Ping: " + pdata.getTransactionPing() + "ms" : ""), false).addField("Date", dtf.format(now), false));
      }
      ...
      try {
         ...
         if (ex.toString().contains("429")) {
            ...
            Karhu.getInstance().getPlug().getLogger().warning("Unable to post discord webhook: " + ex.getMessage());
         }
      }
