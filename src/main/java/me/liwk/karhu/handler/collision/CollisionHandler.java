...
public final class CollisionHandler implements KarhuHandler {
   ...
   public void handle(boolean moved) {
      ...
      BoundingBox mcpAnyLiquidBox = this.data
         .getBoundingBox()
         .clone()
         .translate(this.data.deltas.deltaX, this.data.deltas.lastMotionY * 0.8F + 0.6F, this.data.deltas.deltaZ);
      if (this.data.deltas.motionY == 0.0
         && this.data.isOnGroundPacket()
         && this.data.getLocation().getY() - (double)MathHelper.floor_double(this.data.getLocation().getY()) == 0.0625) {
      ...
      if (Karhu.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19)) {
         entitiesUp = new KarhuStream(
            this.data.getBoundingBox().clone().expandMin(0.0, 1.5, 0.0).expandMax(0.0, 0.7, 0.0).expand(0.65, 0.0, 0.65).getCollidingEntities()
         );
      } else {
         entitiesUp2 = new KarhuStream(
            this.data.getBoundingBox().clone().expandMin(0.0, 1.5, 0.0).expandMax(0.0, 0.7, 0.0).expand(0.65, 0.0, 0.65).getCollidingEntitiesNew()
         );
      }
      ...
      if (this.data.isNewerThan13()) {
         ...
         BoundingBox bbCrouch = new BoundingBox(
            this.data,
            location.x - (double)width,
            location.y,
            location.z - (double)width,
            location.x + (double)width,
            location.y + (double)height,
            location.z + (double)width
         );
         this.data
            .setCrouching(
               bbCrouch.getCachedCollidingBlocks(tickCache).isEmpty()
                  && (this.data.isWasSneaking() || !this.data.getLastBoundingBox().getCollidingBlocks().isEmpty())
            );
      }
      ...
      Block sneakBlock = Karhu.getInstance()
         .getChunkManager()
         .getChunkBlockAt(this.data.getLastLocation().toLocation(this.data.getWorld()).add(0.0, -0.2, 0.0));
      ...
      this.data
         .setInsideBlock(
            blocksInsideMiddleBox.any(
               b -> (b.isSolid() || MaterialChecks.WEIRD_SOLID_NO_LIQUID.contains(b))
