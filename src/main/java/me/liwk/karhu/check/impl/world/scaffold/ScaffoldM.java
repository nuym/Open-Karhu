      ...
      if (!this.data.isNewerThan16()) {
         if (packet instanceof BlockPlaceEvent) {
            ...
            if (pos.getX() != -1.0 && (pos.getY() != 255.0 || pos.getY() != -1.0) && pos.getZ() != -1.0) {
               ...
               Vector eyeLocationFixed = new Vector(this.data.getLastLocation().x, this.data.getLastLocation().y + (double)(!this.data.isNewerThan12() ? sneakAmount1_8 : sneakAmount1_13), this.data.getLastLocation().z);
               if (!this.data.isInsideBlock() && !this.data.isInWeb() && !this.data.isOnWeb() && !this.data.isCollidedHorizontally() && !this.data.isRiding() && !this.data.isSpectating() && !this.data.isPossiblyTeleporting()) {
                  if (facePos.dot(MathUtil.getDirection(this.data.getLocation().yaw, this.data.getLocation().pitch)) >= 0.0 && facePos.dot(MathUtil.getDirection(this.data.getLastLocation().yaw, this.data.getLastLocation().pitch)) >= 0.0 && !hitbox.hasPoint(eyeLocation) && !hitbox.hasPoint(eyeLocationFixed) && this.isNotGroundBridging()) {
                     if (++this.buffer > 5.0) {
