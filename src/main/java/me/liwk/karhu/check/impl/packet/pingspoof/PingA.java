      ...
      if (packet instanceof ConnectionHeartbeatEvent) {
         ...
         if (this.suspicious && this.data.getPing() > this.data.getTransactionPing() + 200L) {
            if (++this.violations >= 5.0) {
               this.fail("§f* Spoofed ping\n §f* diff: §b" + Math.abs(this.data.getPing() - this.data.getTransactionPing()) + "\n §f* TRANSAC: §b" + this.data.getTransactionPing() + "\n §f* KEEPAL: §b" + this.data.getPing(), this.getMaxvl(), 40L);
            }
         } else {
