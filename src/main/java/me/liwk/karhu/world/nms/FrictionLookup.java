...
public final class FrictionLookup {
   private static final Map FRICTION_CACHE = new HashMap();

   public static float lookup(KarhuPlayer data) {
      double downLookup = data.getClientVersion().getProtocolVersion() < 573 ? 1.0 : 0.5000001;
      Location loc = new Location(data.getWorld(), (double)MathHelper.floor_double(data.getLastLocation().getX()), (double)MathHelper.floor_double(data.getLastLocation().getY() - downLookup), (double)MathHelper.floor_double(data.getLastLocation().getZ()));
      ...
      if (block != null) {
         ...
         if (data.getClientVersion().getProtocolVersion() < 47 && MaterialChecks.SLIME.contains(material)) {
            ...
            Set list = (Set)(new KarhuStream(FRICTION_CACHE.keySet())).find((s) -> {
               return s.contains(material);
            });
            return (list == null ? 0.6F : (Float)FRICTION_CACHE.get(list)) * 0.91F;
         }
      } else {
         return 0.54600006F;
      }
